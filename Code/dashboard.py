...
def adversarial_losses_confounds(track):

    fig, axes = plt.subplots(nrows=2, ncols=2, figsize=(15, 8))
    
    axes[0,0].plot(track['loss_L'])
    axes[0,0].set_title('Total loss')
    
    axes[0,1].plot(track['loss_signal_L'])
    axes[0,1].set_title('Signal loss')
    
    axes[1,0].plot(track['loss_noise_L'])
    axes[1,0].set_title('Noise loss')

    axes[1,1].plot(track['loss_confounds_L'])
    axes[1,1].set_title('Confounds loss')
...
def adversarial_ffa_motion(ffa_list,batch_size,device,model,noi_list,face_reg,place_reg,ffa_coords):
    # Plots FFA, reconstructions and correlations with regressors
    ffa_batch = ffa_list[0:batch_size,:]
    ffa_batch = torch.tensor(ffa_batch[:,:]).to(device)
    ...
    signal,noise = model.forward(observation.to(device))
    signal = signal.detach().cpu().numpy()[:,:156]
    noise = noise.detach().cpu().numpy()[:,0,:156]
...
def adversarial_ffa_motion2(ffa_list,batch_size,device,model,noi_list,face_reg,place_reg,ffa_coords):
    # Plots FFA, reconstructions and correlations with regressors
    ffa_batch = ffa_list[0:batch_size,:]
    ffa_batch = torch.tensor(ffa_batch[:,:]).to(device)
...
def adversarial_ffa_motion2_ensemble(ffa_list,batch_size,device,all_models,noi_list,face_reg,place_reg,ffa_coords):
    # Plots FFA, reconstructions and correlations with regressors
    ffa_batch = ffa_list[0:batch_size,:]
    ffa_batch = torch.tensor(ffa_batch[:,:]).to(device)
    ...
    signal_array = np.asarray(all_signal)
    signal_median = np.median(signal_array,axis=0)
    noise_array = np.asarray(all_noise)
    noise_median = np.median(noise_array,axis=0)
    ...
    plt.subplot(1,3,2)
    plt.plot(stats.zscore(signal_median[:,:].mean(axis=0)))
    plt.plot(stats.zscore(face_reg),alpha=.5)
    c = np.corrcoef(signal_median[:,:].mean(axis=0),face_reg)[0,1]  # remember to fix this back
    plt.title(f'ffa signal + regs {c:.2f}')
    
    plt.subplot(1,3,3)
    plt.plot(noise[:,:].mean(axis=0))
    plt.plot(face_reg*noise_median[:,:].mean(axis=0).max(),alpha=.5)
    c = np.corrcoef(noise_median[:,:].mean(axis=0),face_reg)[0,1]
    plt.title(f'ffa noise + regs {c:.2f}')

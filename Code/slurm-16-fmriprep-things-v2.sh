#!/bin/bash
#SBATCH --job-name=2things_fmriprep
#SBATCH --output=../Data/things/slurm_files/output_fmriprep-v2_%a.txt
#SBATCH --error=../Data/things/slurm_files/error_fmriprep-v2_%a.txt
#SBATCH --ntasks=1
...
output_dir=../Data/things/fmriprep2
sing_image=../../fmriprep_22_2.sif
fs_lic=../../fs_licence.txt

working_dir=/scratch/aglinska/fmriprep_${this_sub}
mkdir -p $working_dir
echo $working_dir


singularity run --bind /projects/sccn/andromeda1/aglinska/:/projects/sccn/andromeda1/aglinska/,/scratch/aglinska/:/scratch/aglinska/ $sing_image $data_dir $output_dir -w $working_dir participant --participant-label $this_sub --fs-no-reconall --fs-license-file $fs_lic --output-spaces MNI152NLin2009cAsym:res-2 --ignore slicetiming --nthreads 16 --omp-nthreads 8 --mem_mb 128000

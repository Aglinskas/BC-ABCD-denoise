...
def gather_confounds(sub,run,confounds_fname):
    # define filename
    fname = confounds_fname.format(sub=sub,r=run)
    confounds = pd.read_csv(fname,sep='\t')
    return confounds
...
def calculate_correlations(ffa_list,batch_size,device,model,noi_list,face_reg,place_reg,ffa_coords):
    ...
    observation[0,:] = (observation[0,:]-min_val)/(max_val-min_val+eps)
    
    signal,noise = model.forward(observation.to(device))
    signal = signal.detach().cpu().numpy()[:,:156]
    noise = noise.detach().cpu().numpy()[:,:156]

    corr_raw = np.corrcoef(ffa_list[:,:].mean(axis=0),face_reg)[0,1]
    corr_sig = np.corrcoef(signal[:,:].mean(axis=0),face_reg)[0,1]
    return corr_raw, corr_sig

### Compute stable estimate of the signal

def stable_estimate(ffa_list,batch_size,device,all_models,noi_list,face_reg,place_reg,ffa_coords):
    ffa_batch = ffa_list[0:batch_size,:]
    ffa_batch = torch.tensor(ffa_batch[:,:]).to(device)
    ffa_tensor = ffa_batch.unsqueeze(1)
    coords_tensor = ffa_coords.unsqueeze(2)
    coords_tensor = coords_tensor.repeat(1,1,ffa_tensor.shape[2])
    observation = torch.cat((ffa_tensor,coords_tensor.to(device)),axis=1)
    # Extract average FFA response for each model
    all_signals = []
    ...
    # Find Nearest Neighbors (Points closest to the Gaussian mean in 2D)
    distances = cdist([mean_2d], vectors_2d, metric='euclidean')
    nearest_idx = np.argsort(distances[0])[:10]  # Take the 10 closest points
    # Approximate High-Dimensional Mean as Weighted Average

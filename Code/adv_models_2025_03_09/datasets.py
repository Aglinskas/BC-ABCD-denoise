...
class TrainDataset_coords2conv_motion(torch.utils.data.Dataset):
  'Characterizes a dataset for PyTorch'
  def __init__(self, X, Y, gm_coords, confounds):
    X_tensor = torch.from_numpy(X).unsqueeze(1)
    gm_tensor = torch.from_numpy(gm_coords).unsqueeze(2)
    gm_tensor = gm_tensor.repeat(1,1,X_tensor.shape[2])
    self.obs = torch.cat((X_tensor,gm_tensor),axis=1)
    self.noi = Y
    self.gm_coords = gm_coords
    self.motion = torch.permute(torch.tensor(confounds.loc[:,['X','Y','Z','RotX','RotY','RotZ']].values), (1,0))
  ...
  def __getitem__(self, index):
    ...
    s = 2*random.beta(4,4,1)
    noise_aug = s*noise
    noise_aug = torch.cat((torch.from_numpy(noise_aug).unsqueeze(0),self.motion),dim=0)
    
    return observation, noise_aug, coords
    #return observation, noise

class TrainDataset_coords2conv_motion2(torch.utils.data.Dataset):
  'Characterizes a dataset for PyTorch'
  def __init__(self, X, Y, gm_coords, confounds):
    X_tensor = torch.from_numpy(X).unsqueeze(1)
    gm_tensor = torch.from_numpy(gm_coords).unsqueeze(2)
  ...
  def __getitem__(self, index):
    ...
    return observation, noise_aug, self.motion, coords
    #return observation, noise
